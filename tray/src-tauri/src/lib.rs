...
/// Müşteri raporunu (Word/PDF) indirip İndirilenler klasörüne kaydeder ve açar
#[tauri::command]
async fn download_report(app: AppHandle, base: String, name: String, ext: String) -> Result<String, String> {
    if ext != "docx" && ext != "pdf" {
        return Err("Desteklenmeyen dosya türü".into());
    }
    let mut url = reqwest::Url::parse(&format!("{}/api/report/customer.{}", base.trim_end_matches('/'), ext))
        .map_err(|e| e.to_string())?;
    ...
    let client = reqwest::Client::builder()
        ...
        .build()
        .map_err(|e| e.to_string())?;
    let res = client.get(url).send().await.map_err(|e| format!("Bağlantı hatası: {e}"))?;
    if !res.status().is_success() {
        return Err(format!("Sunucu hatası: HTTP {}", res.status().as_u16()));
    }
    ...
    let slug: String = name
        ...
        .trim_matches('-')
        .to_string();
    let file = dir.join(format!("musteri-raporu-{}.{}", if slug.is_empty() { "rapor".into() } else { slug }, ext));
    std::fs::write(&file, &bytes).map_err(|e| format!("Kaydedilemedi: {e}"))?;
