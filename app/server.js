...
// Müşteri raporu: ada göre backup/replikasyon envanteri (REST'ten, kota harcamaz)
function reportName(req, res) {
    const name = typeof req.query.name === 'string' ? req.query.name.trim() : '';
    if (name.length < 2) {
        res.status(400).json({ error: 'En az 2 karakterlik bir müşteri adı girin.' });
        return null;
    }
    return name;
}
...
function exportFilename(name, ext) {
    const slug = name.replace(/[^a-zA-Z0-9_-]+/g, '-').replace(/^-+|-+$/g, '') || 'rapor';
    return `musteri-raporu-${slug}.${ext}`;
}

app.get('/api/report/customer.docx', async (req, res) => {
    const name = reportName(req, res);
    if (!name) return;
    try {
...
app.get('/api/report/customer.pdf', async (req, res) => {
    const name = reportName(req, res);
    if (!name) return;
    try {
